...
단순히 프롬프트를 잘 쓰는 것보다, 전체 파이프라인 구조를 어떻게 설계하느냐가 결과 품질을 결정한다는 점을 체감했다.

특히 같은 입력이라도 scene을 어떻게 쪼개느냐에 따라 이미지 결과가 완전히 달라졌고,  
“텍스트 → 이미지”가 아니라 “텍스트 → 구조 → 이미지” 단계가 필수라는 것을 확인했다.

또한 이미지 생성 과정에서 프롬프트를 계속 수정하는 방식보다,
shot / angle / lighting 같은 고정된 구조 변수를 두는 것이 훨씬 안정적이었다.

마지막으로, 하나의 스타일로 모든 도메인을 처리하려고 하면 결과가 쉽게 붕괴되기 때문에  
travel, product, diary처럼 도메인을 분리하는 것이 필수라는 걸 배웠다.
